...
const MACROS: Array<{ key: MacroKey; label: string; color: string; bg: string; kcalPerGram: number }> = [
  { key: 'carbs', label: 'Carbs', color: '#d97706', bg: 'bg-amber-50 text-amber-700', kcalPerGram: 4 },
  { key: 'protein', label: 'Protein', color: '#2563eb', bg: 'bg-blue-50 text-blue-700', kcalPerGram: 4 },
  { key: 'fat', label: 'Fat', color: '#dc2626', bg: 'bg-red-50 text-red-700', kcalPerGram: 9 },
];
    ...
    <div className="max-w-6xl mx-auto">
      <div className="flex flex-col gap-3 sm:flex-row sm:items-end sm:justify-between mb-6">
        <div>
          ...
          <h1 className="text-3xl font-bold text-gray-950 mt-1">Analysis Results</h1>
        </div>
        {results.mock && (
          <span className="w-fit text-xs bg-amber-100 text-amber-800 px-3 py-1 rounded-full border border-amber-200">
            Demo Data
          </span>
        )}
      </div>
      ...
      <div className="grid grid-cols-1 lg:grid-cols-[minmax(0,1.35fr)_minmax(320px,0.65fr)] gap-6 items-start">
        <section className="space-y-6">
          {activeImage && (
            <div>
              {hasOverlay && (
                <div className="inline-flex rounded-lg border border-gray-200 bg-white p-1 mb-3 shadow-sm">
                  <button
                    type="button"
                    onClick={() => setImageMode('ai')}
                    className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${
                      imageMode === 'ai' ? 'bg-green-600 text-white' : 'text-gray-600 hover:bg-gray-50'
                    }`}
                  >
                    AI output
                  </button>
                  <button
                    ...
                    className={`px-4 py-2 text-sm font-semibold rounded-md transition-colors ${
                      imageMode === 'original' ? 'bg-green-600 text-white' : 'text-gray-600 hover:bg-gray-50'
                    }`}
                  >
                    Original
                  </button>
              ...
              <div className="rounded-lg overflow-hidden bg-gray-100 border border-gray-200 flex items-center justify-center shadow-sm">
                <img
                  src={activeImage}
        ...
        <aside className="space-y-4 lg:sticky lg:top-24">
          <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
            <div className="text-sm font-medium text-gray-500">Total Calories</div>
            <div className="mt-1 text-4xl font-bold text-gray-950">
              {results.total_calories}
              <span className="text-base font-medium text-gray-500 ml-1">kcal</span>
            </div>
          </div>
          ...
          {saved ? (
            <div className="w-full bg-green-100 text-green-800 font-semibold py-4 rounded-lg text-center">
            ...
            <div className="grid grid-cols-2 gap-3">
              <button
                ...
                className="border border-gray-300 text-gray-700 font-semibold py-3 rounded-lg hover:bg-gray-50 transition-colors"
              >
                Try Another
              </button>
              <button
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-4 shadow-sm">
      <div className="flex items-start justify-between gap-4 mb-4">
        <div>
          <h3 className="font-semibold text-gray-950 capitalize">{item.name}</h3>
  ...
  return (
    <div className="bg-white border border-gray-200 rounded-lg p-5 shadow-sm">
      <div className="flex items-start justify-between gap-3 mb-5">
        <div>
          <h2 className="text-lg font-bold text-gray-950">Macro Ratio</h2>
          <p className="text-sm text-gray-500 mt-1">Calories from carbs, protein, and fat.</p>
        </div>
        <div className="text-right">
          <div className="text-sm font-semibold text-gray-900">{summary.totalMacroCalories}</div>
          <div className="text-xs text-gray-500">macro kcal</div>
        </div>
      </div>
      ...
      <div className="flex flex-col sm:flex-row lg:flex-col xl:flex-row items-center gap-5">
        <div className="relative h-44 w-44 rounded-full shadow-inner" style={pieStyle}>
          <div className="absolute inset-10 rounded-full bg-white flex flex-col items-center justify-center border border-gray-100">
            <span className="text-2xl font-bold text-gray-950">{summary.totalGrams}g</span>
          ...
          {summary.parts.map(part => (
            <div key={part.key}>
              <div className="flex items-center justify-between text-sm mb-1">
                ...
                <span className="font-semibold text-gray-950">{part.percent}%</span>
              </div>
